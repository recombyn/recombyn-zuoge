      ...
      >
        ...
        <span className="min-w-0 flex-1 truncate">
          {layerLabel(
            ...
            t('editor.tools.audioGenerator', { defaultValue: 'Èü≥È¢ëÁîüÊàêÂô? }),
            t
          )}
        </span>
      </button>
...
/** Left layers dock ‚Ä?history + frames/nodes from unified stackOrder. */
function LayerPanel({
  onClose,
  onSelectNode,
  onSelectFrame,
  ...
  // Paste bumps documentRevision / sceneRevision ‚Ä?defer layer-list rebuild so the
  // canvas SoA paint stays ahead of the dock at 2k‚Ä?0k nodes.
  ...
  const historyItems = useMemo(() => {
    // Newest first ‚Ä?length only (never subscribe to full snap docs).
  ...
  return (
    ...
    >
      ...
      {/* Layer rows ‚Ä?top of list = front of stack (virtualized) */}
      <VirtualList
        ref={layerListRef}
        items={layerRows}
