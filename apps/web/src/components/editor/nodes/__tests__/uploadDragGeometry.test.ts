...
describe('upload drag geometry', () => {
  ...
  it('ensureAnimationFrameMedia skips while geometry preview is active', () => {
    let state = seed();
    state = reducer(state, spawnAnimationBoard({ x: 0, y: 0, width: 320, height: 240 }));
  ...
  it('upload on animation workbench: progress + ensureMedia during drag does not snap host', () => {
    let state = seed();
    state = reducer(state, spawnAnimationBoard({ x: 50, y: 60, width: 400, height: 300 }));
    const frameId = String(state.selectedFrameIds[0] || '');
    state = reducer(state, ensureAnimationFrameMedia({ frameId }));
    const hostId = hostIdForFrame(state, frameId)!;
    ...
    state = reducer(
      state,
      patchDocumentNode({ nodeId: uploadId, skipHistory: true, patch: { x: 220, y: 130 } })
    );
    state = reducer(
      state,
      patchDocumentNode({ nodeId: hostId, skipHistory: true, patch: { x: 190, y: 110 } })
    );
