/**
 * SSE design-agent eval with reference images (data URLs).
 * Usage:
 *   node eval/design-agent/ref-ui.mjs
 *   node eval/design-agent/ref-ui.mjs home
 *   node eval/design-agent/ref-ui.mjs detail
 */
...
const API = process.env.EVAL_API || 'http://127.0.0.1:8000';
const OUT = process.env.EVAL_OUT || path.join(root, '.tmp-agent-ref-ui-sse.json');
const token = (
...
async function runCase(c) {
  ...
  let buf = '';
  const deadline = Date.now() + Number(process.env.EVAL_CASE_MS || 480_000);
